...
from alternate_funcs import use_case_tester_dis
import cvxpy as cp
...
fig.savefig("Digital_Futures/data/method2/dissatisfaction/pcc_comparison.pdf", format="pdf")
...
f = use_case_tester_dis(G, H, w, y_ref, 1, U1, U2, U3, U4, U5, U6, t_s=t_s)
...
for k in range(num_samples):
    ...
    objective = cp.Minimize(0.5 * beta * cp.sum_squares(diff) + u_k)
...
x2, test_coeffs= projected_online_gradient_control_sys_id_ARM(f, [1, 5], e_threshold = 1e-10, e_threshold2 = 1e-10, delta = 0.13, f_factor1 = 0.1, f_factor2 = 0.1, win_size1=4, win_size2=9, step=step, sets=sets, x_0=x0)
...
plt.tight_layout()
plt.savefig("Digital_Futures/data/method2/dissatisfaction/tracking_error_plot.pdf")
plt.show()
...
plt.title(rf"Online Optimization Compared to Constrained Optimum")
plt.tight_layout()
plt.savefig("Digital_Futures/data/method2/dissatisfaction/cumulative_tracking_error_plot.pdf")
plt.show()

if save_data == True:
    np.savez("Digital_Futures/data/method2/dissatisfaction/tracking_error_data.npz",
            t=t,
            t_max=t_max,
            t_s=t_s,
