...
from SYSID import calculate_norm_error
from hessian_change import RLS_func, online_gradient_control_sys_id, online_gradient_control_sys_id_ARM
...
L, mu = 8, 3 # smoothness and strong convexity parameters of the cost
...
#x_opt = np.hstack([-la.solve(A_list[k], b) for k in range(num_samples)])
...
x, test_coeffs, all_coeffs_list_ARM, used_index_ARM, norm_list_ARM = online_gradient_control_sys_id_ARM(f, b, [mu, L], delta, lambda_factor, step, A_tilde, x_0=0)
...
plt.semilogy(t[1:], error_control_sys_id_ARM[1:], label="Control_with_sysID_ARX")
